-- Criar política para permitir que a função handle_new_user crie accounts
CREATE POLICY "System can create accounts during user creation" 
ON public.accounts 
FOR INSERT 
TO public
WITH CHECK (true);

-- Atualizar a função handle_new_user para usar SECURITY DEFINER adequadamente
...
BEGIN
  ...
  IF target_account_id IS NULL THEN
    ...
    -- Criar nova conta organizacional (com SECURITY DEFINER, bypassa RLS)
  ...
  -- Inserir perfil do novo usuário (com SECURITY DEFINER, bypassa RLS)
